...
import { MobilePhonePageFrame } from './MobilePhoneShell'
...
export function AdminLayout({ children, activeTab }: AdminLayoutProps) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(168,85,247,0.18),_transparent_30%),linear-gradient(180deg,#09090f_0%,#11111b_45%,#171722_100%)] text-slate-50">
      ...
      <header className="sticky top-0 z-40 hidden border-b border-white/10 bg-slate-950/70 backdrop-blur-xl xl:block">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-4 lg:px-6">
          <div>
            <div className="text-xs tracking-[0.35em] text-violet-200/80 uppercase">
              Super Admin
            </div>
      ...
      <div className="relative mx-auto max-w-7xl px-0 pt-0 pb-0 xl:px-6 xl:pt-5 xl:pb-12">
        <MobilePhonePageFrame
          activeTab="admin"
          title="Super Admin"
          subtitle="Quản trị hệ thống"
          contentClassName="bg-slate-950/70 text-slate-50"
        >
          <section className="mb-6 grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
            <div className="rounded-[2rem] border border-white/10 bg-white/7 p-5 shadow-2xl shadow-black/20 backdrop-blur-xl">
              <p className="text-xs tracking-[0.35em] text-violet-200/80 uppercase">Tổng quan</p>
              <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
                ...
                <div className="text-right text-sm text-slate-300">
                  ...
                  <div>
                    {workspace ? getWorkspaceTypeLabel(workspace.type) : 'Chưa có workspace'}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-[2rem] border border-white/10 bg-slate-900/80 p-5 shadow-2xl shadow-black/20 backdrop-blur-xl">
              <p className="text-xs tracking-[0.35em] text-slate-400 uppercase">Quyền truy cập</p>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 hidden border-t border-white/10 bg-slate-950/90 px-3 py-3 backdrop-blur-xl md:hidden">
